	...
	public function setVirtualPad(virtualPad:FlxVirtualPad, ?DPad:FlxDPadMode, ?Action:FlxActionMode) 
	{
		...
		switch (DPad)
		{
			...
			case DUO:
				...
				inline forEachBound(Control.UI_UP, (action, state) -> addbutton(action, virtualPad.buttonUp2, state));
				inline forEachBound(Control.UI_DOWN, (action, state) -> addbutton(action, virtualPad.buttonDown2, state));
				inline forEachBound(Control.UI_LEFT, (action, state) -> addbutton(action, virtualPad.buttonLeft2, state));
				inline forEachBound(Control.UI_RIGHT, (action, state) -> addbutton(action, virtualPad.buttonRight2, state));                        
			case NONE:
		}
